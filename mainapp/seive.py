...
# Function to read the sieve data via a function
...
def perform_analysis(data, owner_name, init_wt):
    ...
    if df is not None and df is not False:
        ...
        try:
            gravel_percentage = (
                100 - df[df["Seive size (mm)"] == 4.750]["%passing"].iloc[-1]
            )
        except:
            gravel_percentage = 0
        try:
            sand_percentage = (
                100 - df[df["Seive size (mm)"] == 0.0750]["%passing"].iloc[-1]
            )
        except:
            sand_percentage = 0
        try:
            silt_clay_percentage = round(
                df[df["Seive size (mm)"] == 0.0750]["%passing"].iloc[-1], 3
            )
        except:
            silt_clay_percentage = 0
        # Write DataFrame to Excel file
        file_name = f"{owner_name}.xlsx"
        local_path = f"media/{file_name}"
        df.to_excel(local_path, index=False)
        # Upload to S3
        bucket_name = settings.AWS_STORAGE_BUCKET_NAME
        s3_url = upload_to_s3(local_path, bucket_name, file_name)
        if s3_url:
            ...
            dic = {
                ...
                "CU": CU,
                "CC": CC,
                "gravel_percentage": gravel_percentage,
                "sand_percentage": sand_percentage,
                "silt_clay_percentage": silt_clay_percentage,
                "df_eive": df["Seive size (mm)"],
                "df_passing": df["%passing"],
            }
            return dic
        else:
            return {"detail": "Failed to upload file to S3"}
    return df
